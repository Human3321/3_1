...
int rw_mutex =1; //세마포어 변수 선언
int mutex =1;//세마포어 변수 선언
...
void* read_th(void *arg) //read 스레드
{
	...
	while(1)
	{
		...
		now_access++;//현제 접근한 스레드 증가
		if(now_access==1) //접근 스레드가 1이면
			wait(rw_mutex);//write함수가 unlock해줄때때까지 대기
		signal(mutex);//read에서 동작하는 unlock
		//크리티컬 섹션 시작
		...
		i=0;
		//크리티컬 섹션 종료
		wait(mutex);//read에서 동작하는 lock
		now_access--;//read에 접근한 함수 카운트 감소
		if(now_access==0) //현제 읽고있는 스레드가 0개면
			signal(rw_mutex);//write를 umlock하여서 작동
